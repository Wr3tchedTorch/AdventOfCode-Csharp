...
{
    readonly int diskSpace = 70000000;
    readonly int minSpaceNeeded = 30000000;
    ...
    public int GetSizeOfDeletableDir()
    {
        int total = 0;
        int spaceNeeded = minSpaceNeeded - (diskSpace - directories["/"]);
        int lowestDirFound = int.MaxValue;
        foreach (int size in directories.Values)
        {
            if (size < lowestDirFound && size > spaceNeeded)
            {
                lowestDirFound = total = size;
            }
        }
        return total;
    }
